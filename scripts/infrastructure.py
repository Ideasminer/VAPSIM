class Spot():
    def __init__(self, *args):
        ...
        if args:
            if len(args) < 2:
                raise ValueError(
                    "Expects at least 2 args: name(int) and centriod(tuple)")
            else:
                self.name = args[0]
            ...
            raise ValueError(
                "Need at least 2(name, centriod), at most 4(name, centriod, occupy, size)")
    ...
    def get_border(self, size):
        ...
        # rt: right-top, rb: right-bottom, lb: left-bottom, lt: left-top
        return [rt, rb, lb, lt]
...
class Stack(Spot):
    def __init__(self, *args, **kwargs):
                    ...
                    if "occupy" in paramNums:
                        if "size" in paramNums:
                            super(kwargs.get("name"), kwargs.get("centriod"),
                                  kwargs.get("occupy"), kwargs.get("size"))
                        else:
                            super(kwargs.get("name"), kwargs.get(
                                "centriod"), kwargs.get("occupy"))
                    else:
                        super(kwargs.get("name"), kwargs.get("centriod"))
                else:
        ...
        self.border = self.get_border(
            (self.size[0], self.size[1] * self.containerNum))
    ...
    def auto_generate(self):
        ...
        if direction:  # direction == 1 +x
            start_centriod = (
                stack_centriod[0] + (float(num) - 1) / 2 * size[1], stack_centriod[1])
        else:
            start_centriod = (
                stack_centriod[0] - (float(num) - 1) / 2 * size[1], stack_centriod[1])
        for i in range(num):
...
class Area(Stack):
    def __init__(self, *args, **kwargs):
        ...
        self.border = self.get_border(
            (self.size[0] * self.containerNum, self.size[1] * self.baseNum))
        self.containerList = self.auto_generate()
        self.occupy = sum([i.occupy for i in self.containerList])
    ...
    def auto_generate(self):
        ...
        if direction:  # direction == 1 +y
            start_centriod = (
                stack_centriod[0], stack_centriod[1] + (float(num) - 1) / 2 * size[0])
        else:
            start_centriod = (
                stack_centriod[0], stack_centriod[1] - (float(num) - 1) / 2 * size[0])
        for i in range(num):
            ...
            if direction:
                ...
                centriod = (start_centriod[0], start_centriod[1] + size[0])
            occupy = 0
            newContainer = Stack(name, centriod, occupy, self.size,
                                 direction=self.stack_direction, containerNum=self.baseNum)
            containerList.append(newContainer)
        return containerList
...
class Lot():
    def __init__(self, *args):
        ...
        if args:
            if len(args) != 3:
                raise ValueError(
                    "All params are required: parking matrix, corridor matrix and in-out lane number")
            else:
                self.parking, self.corridor, self.outway = args
        else:
            raise ValueError(
                "All params are required: parking matrix, corridor matrix and in-out lane number")
        self.occupy = [[0 for j in range(self.parking[i][0])]
                       for i in range(len(self.parking))]
    ...
    def area_generate(self, spot_size, corridor_width, area_direction):
        ...
        for i in range(area_num):
            ...
            if i == 0:
                ...
                start_x = parking[i - 1][1] * spot_size[0] + \
                    sum(self.corridor[i - 1]) * corridor_width
                x = spot_num / 2 * spot_size[1] + start_x
                y = stack_num / 2 * spot_size[0]
                centriod = (x, y)
            occupy = 0
            newArea = Area(name, centriod, occupy, spot_size, baseNum=spot_num,
                           containerNum=stack_num, direction=area_direction, stack_direction=stack_direction)
        ...
        for i in range(len(self.occupy)):   # Area i
            for j in range(len(self.occupy[i])):         # Stack j in Area i
                ...
                if stack_occupy > len(spotList):
                    raise ValueError(
                        "Wrong assign matrix, value larger than capacity")
                [k_null.updateOccupy(0) for k_null in spotList[:(
                    len(spotList) - stack_occupy)]]
                [k_occu.updateOccupy(1) for k_occu in spotList[(
                    len(spotList) - stack_occupy):]]
